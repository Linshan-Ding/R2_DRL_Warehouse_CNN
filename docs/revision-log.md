...
| R1.2 | 1:1 / 1:n / n:1 配比 | E0+E1+E2 → `tab_ratio.tex` | Rev.1 Comment 2 | §5.7.1（Table 17 + Fig. 15） | **已改稿** |
...
| R1.5 | 载运容量 | E7 + E10 规则扫描 + E9 → `tab_capacity.tex` / `tab_capacity_rules_sweep.tex` / `tab_state_capacity.tex` | Rev.1 Comment 5 | (A1) 一般化 + 新 Eq.(8) 容量约束 + 记号表 + §5.7.2（Table 18 + Fig. 16）| **已定稿**（E9 结果已回填收尾段） |
...
- 合并消融表（Table 20，上块 γ / 下块观测）：保留 **mean ± std（3 次独立训练）**——论证依赖方差与配对检验。
- 补充实验数字只在新流水线内部自洽比较，不与主表（Tables 8/10）逐数字对照。

## 待办

实验类待办已全部完成（E10 于 2026-08-28、E9 于 2026-08-29 回填；图 13/14 已替换）。
round-5 表达润色（2026-08-29）：算例表并入 §5.1 成 Table 6，Table 19/22 撤表改行内，
Tables 20+21 合并为 Table 20，新增 Fig. 15/16，正文与回复信分号清零、四处返修口吻
中性化（§3.1/§5.4/§5.6/§5.8.2），回复信全部编号页码按新 aux 重锚。剩余均为人工项：
...
- 回复信与论文补充实验表格的所有数字由 `paper_assets/make_tables.py` 从
  ...
  是验收标准（round-5 后论文成表片段 5/5 通过：ratio/capacity/picktime/
  ablation/training_cost；行内化数字与 Fig. 15/16 图内数值与片段同源）。
- 审稿意见在回复信中逐字照录（11/11）。
