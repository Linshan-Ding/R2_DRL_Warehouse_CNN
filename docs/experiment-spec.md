...
| E4 | `run_e4_gamma.py` | γ ∈ {0.95,0.99,1.00} | 各 3 | 2000 | SAPPO | Table 21 上块（§5.8.1，mean±std） |
| E5 | `run_e5_state_channels.py` | state_channels=plus_agent | 3 | 2000 | SAPPO | Table 21 下块（§5.8.2，mean±std+配对 t） |
| E6 | `run_e6_picktime.py` | τ_pick ∈ {15,20} s | 各 1 | 2000 | SAPPO+5 规则 | Table 20（§5.7.3） |
...
| **E10** | `run_e10_rules_capacity.py` | C ∈ {1..5}，仅规则 | 1 | 免训练 | 5 规则 | Table 19（§5.7.2 规则容量扫描） |
...
- round-5（表达润色）：E8 与 E9 的结果撤表改行内，E4+E5 合并为 Table 21
  （`tab_ablation.tex`），E10 的规则扫描升格为正文 Table 19。
