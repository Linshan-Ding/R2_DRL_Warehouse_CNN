...
修订稿（R2）与回复信在论文仓库 `R2_Jiajin_Li_Dynamic_Order_Picking_CAIE` 维护。
...
> Table 20，浮动体位置参数（`[!htbp]` 等）全部移除，全文分号清零。round-7：容量表
> 与规则扫描表合并为 Table 18（C=1..5），训练成本表去掉 Runs 列，正文与回复信不再
> 陈述重复训练次数，破折号与解释性冒号一并收敛。
...
**已完成（round-9）**：`experiment/run_e7_capacity.py` 的 C=4/5 两个配置已由作者跑完并上传
（`result/e7_c4_run1/`、`e7_c5_run1/`），论文 Table 18 的 SAPPO 行补全为
1557.1 / 1879.5 / 1912.0 / **1994.4** / **2151.1**——单调变差，未出现反超，原结论
（学习策略尚未利用批量化）得到坐实。逐步操作留在 §7 的"E7 补跑 C = 4 / 5"小节作为复跑指南。
...
| `paper_assets/make_tables.py` | 从 result/ 生成论文表格 LaTeX 片段 | 表 16–20 及两张行内数字溯源片段 |
...
| `paper_assets/generated/tab_*.tex` | `paper_assets/make_tables.py` | 论文表 16–20 及两张行内数字溯源片段的 tabular 主体 |
