...
round-5 表达润色（2026-08-29）：算例表并入 §5.1 成 Table 6，布局表与容量-状态表撤表
改行内，γ 与观测两张消融表合并为 Table 21，E10 规则扫描升格为 Table 19，浮动体位置
参数全部移除（避免 Overleaf 编译后出现大块空白），正文与回复信分号清零、四处返修
口吻中性化（§3.1/§5.4/§5.6/§5.8.2），回复信全部编号页码按新 aux 重锚。剩余均为人工项：
