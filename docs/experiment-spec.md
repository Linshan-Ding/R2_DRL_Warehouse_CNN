...
| E1 | `run_e1_ratio.py` | (K,R)=(1,1)/(3,1)/(4,2) | 各 1 | 2000 | SAPPO+5 规则 | Table 17（§5.7.1，另见 Fig. 15） |
...
| E7 | `run_e7_capacity.py` | C ∈ {2,3} | 各 1 | 2000 | SAPPO+5 规则 | Table 18（§5.7.2，另见 Fig. 16） |
...
| **E10** | `run_e10_rules_capacity.py` | C ∈ {1..5}，仅规则 | 1 | 免训练 | 5 规则 | §5.7.2 U 形句 + Fig. 16 + `tab_capacity_rules_sweep.tex` |
...
- round-5（表达润色）：E8 与 E9 的结果撤表改行内，E4+E5 合并为 Table 20
  （`tab_ablation.tex`），新增 `plot_operational_figures.py` 生成 Fig. 15/16。
