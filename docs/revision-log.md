...
| R1.5 | 载运容量 | E7 + E10 规则扫描 → `tab_capacity.tex` / `tab_capacity_rules_sweep.tex` | Rev.1 Comment 5 | (A1) 一般化 + 新 Eq.(8) 容量约束 + 记号表 + §5.7.2（Table 17） | **已改稿**；收尾句待 E9 |
...
| R2.4 | 状态充分性 | E5 → `tab_state.tex`（p = 0.317） | Rev.2 Comment 4 | §4.2–4.3 观测形式化 + 信息表（Table 4）+ §5.8.2（Table 21） | **已改稿**；尾句待 E9 |
...
## 待办（对应 README"接下来需要运行的补充实验"）

1. `experiment/run_e10_rules_capacity.py`（分钟级）→ `paper_assets/make_tables.py`
   → 用 `tab_capacity_rules_sweep.tex` 回填论文 §5.7.2 的 C=4/5 TODO。
2. `experiment/run_e9_capacity_state.py`（GPU，约 6×2.5–4 h）→ `make_tables.py`
   → 按 `tab_state_capacity.tex` 定稿论文 §5.7.2 / §5.8.2 与回复信两处 E9 段落。
3. `paper_assets/plot_warehouse_scale_figures.py`（秒级）→ 用修正标注后的
   `Aisles.pdf` / `Rack Capacity.pdf` 覆盖论文 `Figure/` 目录。
4. **待作者**：重绘 Fig. 2（depot + 两类示例路径）；填 Manuscript ID；
